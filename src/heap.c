		...
		if (header->pd_linp[i].lp_len != 0)
		{
			ereport(WARNING,
					(errmsg("[%d:%d] tuple with LP_REDIRECT and len != 0 (%d)",
							block, (i+1), header->pd_linp[i].lp_len)));
			++nerrs;
		}
		...
		if (header->pd_linp[i].lp_off <= 0)
		{
			ereport(WARNING,
					(errmsg("[%d:%d] tuple with offset <= 0 (%d)",
							block, (i+1), header->pd_linp[i].lp_off)));
			++nerrs;
		}
		...
		if (header->pd_linp[i].lp_off < header->pd_upper)
		{
			ereport(WARNING,
					(errmsg("[%d:%d] tuple with offset - length < upper (%d - %d < %d)",
							block, (i+1), header->pd_linp[i].lp_off,
							header->pd_linp[i].lp_len, header->pd_upper)));
			++nerrs;
		}
		...
		if (header->pd_linp[i].lp_off + header->pd_linp[i].lp_len > header->pd_special)
		{
			ereport(WARNING,
					(errmsg("[%d:%d] tuple with offset > special (%d > %d)",
							block, (i+1), header->pd_linp[i].lp_off, header->pd_special)));
			++nerrs;
		}
	}
	...
	else
	{
				...
				att_isnull(j, tupheader->t_bits))
			{
				ereport(DEBUG3,
						(errmsg("[%d:%d] attribute '%s' is NULL (skipping)",
								block, (i+1), rel->rd_att->attrs[j]->attname.data)));
				continue;
			}
				...
				if (len < 0)
				{
					ereport(WARNING,
							(errmsg("[%d:%d] attribute '%s' has negative length < 0 (%d)",
									block, (i+1), rel->rd_att->attrs[j]->attname.data, len)));
					++nerrs;
					break;
				}
						...
						(VARRAWSIZE_4B_C(buffer + off) > 1024*1024))
					{
						ereport(WARNING,
								(errmsg("[%d:%d]  attribute '%s' has invalid length %d (should be between 0 and 1G)",
										block, (i+1), rel->rd_att->attrs[j]->attname.data, VARRAWSIZE_4B_C(buffer + off))));
						++nerrs;
		...
		ereport(DEBUG3,
				(errmsg("[%d:%d] last attribute ends at %d, tuple ends at %d",
						block, (i+1), off, header->pd_linp[i].lp_off + header->pd_linp[i].lp_len)));
