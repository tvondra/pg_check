...
check_page_header(PageHeader header, BlockNumber block)
{
	...
	/*
	 * check that a page is new, and don't do any further checks for such
	 * pages
	 */
	if (PageIsNew(header))
	{
		 ...
		 *
		 * XXX Maybe we need to invent a different way to pass info that it's
		 * safe to perform further checks on the page?
		 */
		return nerrs;
	}

	/*
	 * All the pointers should be positive (greater than PageHeaderData) and
	 * less than BLCKSZ.
	 */
	...
	/*
	 * PostgreSQL up to 9.2 had pd_tli in the header, with page timeline. 9.3
	 * replaced that with a checksum, so be careful when reading and
	 * interpreting that value.
	 *
	 * We only check the timeline here, because we only deal with page headers
	 * here. Checksums are verified elsewhere.
	 */
